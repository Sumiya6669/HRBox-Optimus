...
## 13. Доступ к разделам

Доступ решается в ДВУХ слоях, и путать их нельзя.

**Слой безопасности — RLS в базе.** Кто чью зарплату, заявку или операцию может
прочитать, определяют политики на таблицах. Этот слой обойти нельзя: он работает
на любом запросе, хоть из интерфейса, хоть из curl.

**Слой удобства — `role_permissions`.** Какие разделы роль видит в меню и куда её
пускает роутер. Настраивается администратором в `/admin/permissions`. Этот слой
НЕ защищает данные: он живёт в интерфейсе, и его достаточно обойти прямым
запросом к API. Закрывать им доступ к чувствительным данным — ошибка.

Практические правила:

* каталог разделов — только `src/lib/sections.js`. Раньше доступ был размазан по
  трём местам (маршрут, пункт меню, проверка внутри страницы), и они расходились:
  пункт меню пропадал, а страница открывалась по прямой ссылке;
* решение о доступе принимает одна чистая функция `evaluateAccess()`. На неё
  опираются меню (`PortalShell`), роутер (`RequireAuth section=...`) и тесты;
* у каждого раздела есть `minRole` — жёсткий пол, который администратор снять не
  может. Иначе галочка была бы обманом: доступ выдан, а RLS всё равно откажет, и
  человек попадёт на экран с ошибкой вместо раздела;
* **отсутствие записи в `role_permissions` означает «разрешено»**, а не
  «запрещено». Иначе каждый новый раздел в коде оказывался бы закрыт для всех до
  первого сохранения настроек — включая администратора, который их и правит;
* разделы `admin.users`, `admin.permissions`, `admin.settings` для роли `admin`
  заблокированы триггером в базе. Сняв их, администратор потерял бы единственный
  способ вернуть права, и чинить пришлось бы через SQL-редактор;
* новый экран портала — это новая запись в `SECTIONS`, ключ в `perm:` у пункта
  меню и обёртка `gate('ключ', <Page/>)` у маршрута. Три места, но источник
  правды один.

## 14. Заведение учётных записей

Способов три, и ни один не покрывает всё:

* **сразу с паролем** (`create-user`) — администратор задаёт логин и пароль и
  передаёт их лично. Единственный вариант, когда у человека нет рабочей почты
  или учётка нужна немедленно;
* **ссылка-приглашение** (`create_invitation` → `accept-invite`) — человек сам
  задаёт пароль. Открытый токен возвращается ровно один раз, в базе только хеш;
* **письмо** (`invite-user`) — требует настроенного SMTP. Встроенный почтовик
  Supabase шлёт несколько писем в час и на приём группы не годится.

Общее для всех трёх: создание учётки идёт ТОЛЬКО через Edge-функцию с
service_role-ключом, который никогда не попадает в клиентский бандл, и право
приглашать проверяется по роли вызывающего в `profiles`, а не по телу запроса.

Вместе с учёткой заводится карточка сотрудника. Без неё личный кабинет пустой:
KPI, цели, отпуск и уведомления привязаны к `employee_id`, а не к учётной записи.

## 15. Курсы и тестирование

**Курс = уроки + (необязательно) итоговый тест.** Уроки лежат в
`course_lessons`, прохождение — в `lesson_progress`, тест — в `course_tests` /
`test_questions` / `test_options`, попытки — в `test_attempts`.

**Прогресс считает база, не браузер.** `enrollments.progress` пересчитывает
триггер по доле пройденных ОБЯЗАТЕЛЬНЫХ уроков. Клиентский подсчёт означал бы,
что цифра зависит от того, дождалась ли страница ответа, и «100 %» появлялось бы
у человека, который просто пролистал уроки. Курс переходит в `completed`, только
когда пройдены все обязательные уроки И сдан тест, если он есть.

**Правильные ответы не покидают сервер.** Это главное правило раздела:

* RLS на `test_options` отдаёт строки только HR. Сотрудник не читает эту таблицу
  вообще — в ней флаг `is_correct`;
* вопросы приходят через `start_test_attempt()`, которая собирает варианты
  БЕЗ этого флага;
* ответы проверяет `submit_test_attempt()` в базе, по идентификаторам вариантов;
* разбор ответов отдаётся только если у теста включён `show_correct`, и только
  после закрытия попытки.

Если когда-нибудь понадобится отдать вопросы обычной выборкой — это ошибка:
ключ окажется в консоли разработчика за десять секунд.

**Остальные правила теста:**

* дедлайн вычисляется на сервере и хранится в попытке (`deadline_at`). Таймер в
  браузере — только индикатор: перезагрузка страницы время не обнуляет;
* лимит попыток считает только ЗАВЕРШЁННЫЕ попытки — упавший интернет не должен
  стоить человеку попытки;
* вопрос засчитывается при полном совпадении множеств ответов. Частичный балл не
  начисляется сознательно: «угадал два из трёх» в проверке по технике
  безопасности не половина успеха;
* ответы из браузера приводятся к массиву явно. Пропущенный вопрос приходит как
  `null`, и `jsonb_array_elements_text` на скаляре роняет всю отправку — вместе с
  ней пропала бы вся работа человека;
* сравнение «своя ли попытка» — через `is distinct from`. Обычное `<>` с NULL
  даёт NULL, и пользователь без карточки сотрудника прошёл бы мимо проверки.

**Видео грузится файлом.** Бакет `course-media` (500 МБ, видеоформаты) отдельно
от `portal-files` (25 МБ, без видео). Ссылка на внешний ролик оставлена запасным
вариантом, но по умолчанию — файл: чужой хостинг переименует или закроет видео,
и обязательный курс молча перестанет открываться.

## 16. Сессия и потеря введённых данных

**Профиль перечитывается ТОЛЬКО при смене пользователя.**

Это не оптимизация, а исправление ошибки. `supabase.auth.onAuthStateChange`
присылает события не только при реальном входе: возвращаясь на вкладку, открывая
свёрнутое окно или просыпаясь после сна, браузер заново проверяет сессию и
получает `SIGNED_IN` на того же самого пользователя. Старый код на любое событие
ставил `isLoadingAuth = true`, `RequireAuth` показывал загрузчик, и React
размонтировал всё поддерево страницы — открытая форма, набранный текст и
выбранные галочки пропадали. Пользователи описывали это как «страница сама
обновляется».

Поэтому в `AuthContext` есть `loadedForUser` — ref с id уже загруженного
пользователя. Если id тот же, обработчик выходит немедленно.

**Не добавляйте в обработчик событий побочных эффектов, ведущих к загрузчику.**
Любой `setIsLoadingAuth(true)` вне смены пользователя вернёт ту же ошибку.

**Длинные формы пишут черновик** через `useFormDraft` (`src/lib/useFormDraft.js`)
в `sessionStorage`. Именно session, а не local: данные живут до закрытия вкладки
и не всплывают через неделю на чужом компьютере. Пароли и другие секреты в
черновик не кладём — они держатся в отдельном `useState`.

## 17. Загрузка больших файлов

Видео идёт через **возобновляемую загрузку (tus)** кусками ровно по 6 МБ —
такой размер требует само хранилище Supabase, менять его нельзя. Обычный POST
отправляет файл одним куском: на 500 МБ это десятки минут, и любой обрыв
начинает всё заново, поэтому на нестабильной связи загрузка не заканчивается
никогда. Файлы до 6 МБ отправляем обычным способом — возобновление им ничего не
даёт.

`onProgress` обязателен для файлов такого размера: без полосы прогресса человек
через минуту решает, что всё зависло, и перезагружает страницу.

**Потолок задаёт тариф, а не код.** На Free-плане Supabase максимум 50 МБ на
файл — это жёсткое ограничение сервиса, никакой настройкой бакета его не обойти.
На Pro — до 500 ГБ, но глобальный лимит в Settings → Storage надо поднять руками.
